...
class DiceSession:
    def __init__(self):
        ...
        # Cle API Mistral : vide = mode manuel (bouton "copier le prompt"
        # existant) ; non vide = l'appli envoie chaque evenement a l'IA et
        # affiche sa reponse directement, sans copier-coller.
    ...
    def load(self):
        if os.path.exists(SAVE_FILE):
            ...
            self.mistral_api_key = data.get("mistral_api_key") or ""
    ...
    def set_mistral_key(self, key):
        self.mistral_api_key = (key or "").strip()
        self.save()

    def clear_mistral_key(self):
        self.mistral_api_key = ""
        self.save()

    def has_mistral_key(self):
    ...
    def second_souffle(self):
        """Effet special de la Patte (Emblazon d'Animorph) : retire le
        dernier lancer de reussite (ou mixte) de l'historique et relance
        immediatement le de de reussite a sa place."""
        for i in range(len(self.history) - 1, -1, -1):
            if self.history[i]["type"] in ("success", "both"):
                self.history.pop(i)
                break
        return self.roll_success("Second Souffle (Emblazon d'Animorph)")
    ...
    def reset_all(self):
        ...
        avec l'IA. Pratique pour repartir d'une aventure neuve apres un
        test. La cle API Mistral est preservee (c'est un reglage de
        l'appli, pas une donnee de l'aventure) : pas besoin de la recoller
