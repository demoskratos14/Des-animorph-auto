...
from bg_animorph_data import BG_IMAGE_B64
...
"""
BASE_CSS = BASE_CSS.replace("__BG_IMAGE_B64__", BG_IMAGE_B64)
...
# Texte declenche quand on depense la jauge d'energie pleine d'un allie
# (araignee/bouclier/etoile n'ont pas de fiche TOTEMS, ce sont des allies).
ALLY_HELP_TEXT = {
...
def layout(title, body):
    ...
    <div class="header-block">
      <h1>Les Des de l'Aventure d'Animorph</h1>
      {TOTEM_ROW_HTML}
...
def run_ai_narrator(event_text):
    ...
    if not session.has_mistral_key() or not event_text:
        return None, None
    ...
    text, error = mistral_client.chat(session.mistral_api_key, session.ai_messages_to_send())
    if error:
        return None, error
...
def render_ai_panel_html(transient_error=None):
    ...
    if not session.has_mistral_key():
        return (
            '<div id="aiStoryPanel">'
...
def build_mechanics_context(auto_mode=False):
    ...
    if auto_mode:
        lines.append("Narrateur d'une histoire heroique interactive, 2e personne ('tu'), "
                      "adressee a Gabin/Animorph. Mecaniques ci-dessous ; l'histoire se "
                      "poursuit directement dans cette conversation, evenement par evenement.")
    else:
        lines.append("Narrateur d'une histoire heroique interactive, 2e personne ('tu'), "
                      "adressee a Gabin/Animorph. Mecaniques ci-dessous, puis l'histoire deja vecue.")
    lines.append("")
    ...
    lines.append(
        ...
        f"{PIP_SYMBOLS['etoile']['emoji']}Etoile=allie Shuri (Wakanda) | "
        f"{PIP_SYMBOLS['patte']['emoji']}Patte (Emblazon d'Animorph)=pas d'allie exterieur, "
        "declenche 'Second Souffle' (relance immediatement le dernier lancer de reussite)."
    )
    lines.append(
    ...
    if auto_mode:
        lines.append(
            ...
            "et tu attends le resultat suivant avant de continuer ; tu t'adresses "
            "toujours a Gabin/Animorph en 'tu'."
        )
    else:
        lines.append(
            ...
            "pour garder une trace permanente ; tu t'adresses toujours a "
            "Gabin/Animorph en 'tu'."
        )
    return "\n".join(lines)
...
def do_use_totem_energy():
    ...
    if spent:
        if key == "patte":
            session.second_souffle()
            effect_text = ("\U0001f504 Second Souffle ! Le dernier lancer de reussite "
